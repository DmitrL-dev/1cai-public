...
В ветке `codex/snapshot-observer-dev8` подготовлены сравнение снимков и
...
Dev8 ещё не заменяет опубликованную dev7 и профиль companion 0.1.4.
...
В dev8 подтверждены [регистрация профиля и запуск предложения через установленный CLI](TEST-PROFILES.md): две отдельные базы, fail/pass, восстановление результата и запрет нового запуска отключённого профиля. Интеграция тестов в редактор и типовые конфигурации пока не приняты.
...
| Ежедневная разработка dev8 | AI-правка, ручное исправление новой ревизией, конфликт версий, нативная проверка и восстановление проверены в редакторе | [AI-ошибка сохраняется](DAILY-DEVELOPMENT-ACCEPTANCE.md); [исправление человеком проверено](MANUAL-DRAFT-EDIT.md). Бизнес-тесты и применение не приняты |
...
| Observer | В dev8: опрос выгрузки, журнал, восстановление, отчёт об изменениях | Нет Git watcher, жизненного цикла находок и общей приёмки O1 |
