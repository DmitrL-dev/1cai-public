...
успешно, а запись по пути уже проверенно удалённого потомка появилась в первом
перечислении родителя, затем отсутствовала в повторном и новом;
повторное открытие вернуло WinError `2`. Идентичность этой записи после
удаления с исходным объектом не сравнивалась. [Квитанция](evidence/service-git-cleanup-trace-dev11-20260925.json)
