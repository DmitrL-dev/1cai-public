...
| --- | --- | --- |
| Core 0.1.0.dev10 | Всё принятое в dev9; ASGI-приём `POST /notifications`, durable digest-квитанция, локальная TLS-проверка установленного wheel и отдельная нативная проверка BSL apply/Undo после выпуска | Требует отдельно настроенного Uvicorn/httptools host; публичный HTTPS/DNS/SCM deployment, типовые конфигурации и живая ИБ не приняты |
