...
[Дополнительная трасса холодной очистки](SERVICE-GIT-CLEANUP-TRACE-DEV11.md)
на новых проектах обнаружила ещё один публичный первый `fatal` без
зафиксированного внутреннего `reason` и несколько отказов локальной
диагностической копии с `BSL_CLEANUP_FAILED`. В одном отказе Java завершилась
успешно, а запись по пути потомка, ранее прошедшего `delete_verified`
(пометку на удаление), появилась в первом перечислении родителя, затем
отсутствовала в повторном и новом; повторное открытие вернуло WinError `2`.
Идентичность этой записи с помеченным объектом не сравнивалась.
[Квитанция](evidence/service-git-cleanup-trace-dev11-20260925.json)
связывает первые циклы, хэши полных FileStamp 579 объектов runtime и
отдельный прерывистый WinError `5` публичного installer. Причина исходного
