...
В текущем исходном кандидате собрано 1301 тестов; Product CI запускается для
...
| Обновления конфигураций | Path-bytes и UUID-aware Designer XML three-way plan с bounded хэшами, явными конфликтами и evidence по прямым свойствам (`same_change`/`disjoint_changes`/`overlap_conflict`); для безконфликтных path-level решений доступен in-memory candidate с digest и fail-closed конфликтами | Property/extension/BSL merge semantics, тестовая база, backup/rollback и запись не приняты |
