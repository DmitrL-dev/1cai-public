...
| Observer | Опрос выгрузки, журнал и восстановление; Git probe, durable lifecycle находок, per-commit snapshot binding для owner report, bounded scheduler и foreground service-host adapter с journal/recovery и atomic notification outbox, read-only BSL-LS Git adapter с clean-commit/HEAD/ancestry recheck | Нет SCM-инсталлятора/фонового service manager, внешнего канала доставки, доказательства commit↔snapshot содержимого, полной native-приёмки analyzer на типовой конфигурации и общей приёмки O1 |
