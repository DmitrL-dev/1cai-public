...
| Observer | В dev8: опрос выгрузки, журнал, восстановление, отчёт об изменениях; Git probe и durable lifecycle находок с replay/resolve/reopen подключены к SQLite API, запись и чтение находок доступны через observer CLI | Нет автоматического Git watcher/scheduler, запуска анализатора, MCP-интеграции и общей приёмки O1 |
