...
| Observer | В dev8: опрос выгрузки, журнал, восстановление, отчёт об изменениях; отдельная библиотека Git probe и lifecycle находок подготовлена | Git probe не подключён к watcher/CLI/MCP, нет durable Git-журнала и общей приёмки O1 |
