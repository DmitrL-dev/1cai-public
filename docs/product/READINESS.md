...
| Реальная платформа 1С | 8.3.27.2342: создание базы, XML → изменение имени → загрузка → обновление базы → проверка → XML | Пустая собственная конфигурация; [полная приёмка не завершена](PLATFORM-ACCEPTANCE.md) |
