"""Verify proposal tests in an actual companion extension host; no model calls."""
import argparse
import json
import os
...
def run(args):
    ...
    env.pop("RENTGEN_TESTS_ACCEPTANCE_PRIOR", None)
    env.update(
        RENTGEN_EDITOR_PROFILE=str(root),
        RENTGEN_TESTS_ACCEPTANCE_OUTPUT=str(output / "acceptance.json"),
    )
    if args.prior:
        env["RENTGEN_TESTS_ACCEPTANCE_PRIOR"] = str(args.prior.resolve(strict=True))
    script = Path(__file__).with_name("companion_host") / "tests.cjs"
    command = [
    ...
    with (output / "host.log").open("w", encoding="utf-8") as log:
        ...
        code = wait_owned(process, log, timeout=1100)
...
if __name__ == "__main__":
    ...
    for name in ("profile", "output", "vsix", "editor"):
