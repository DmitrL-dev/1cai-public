...
`companion-v0.1.9`, `core-v0.1.0-dev9`. Тег и уже опубликованные байты не
...
заметки для пользователя. `prepare_companion.py build --tag companion-v0.1.9
...
```powershell
py -3.11 scripts/release/prepare_core.py prepare --version 0.1.0.dev9 --kit PATH_TO_ACCEPTED_KIT.zip --output NEW_DIRECTORY
py -3.11 scripts/release/prepare_core.py verify --version 0.1.0.dev9 --output NEW_DIRECTORY
```
