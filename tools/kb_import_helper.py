...
def create_template(output_path: str, config_name: str = "erp"):
    ...
    print(
        f"  python tools/kb_import_helper.py --import {output_path} --config {config_name}"
    )


def main():
    parser = argparse.ArgumentParser(
        description="Помощник для импорта данных в базу знаний"
    )
    ...
    parser.add_argument(
        "--list", action="store_true", help="Показать список конфигураций"
    )
    ...
    if args.template:
        ...
        for config in kb.SUPPORTED_CONFIGURATIONS:
            ...
            practices_count = (
                len(config_info.get("best_practices", [])) if config_info else 0
            )
            print(f"  - {config}: {config_name}")
            if modules_count > 0 or practices_count > 0:
                print(
                    f"    Модулей: {modules_count}, Best practices: {practices_count}"
                )
    else:
        parser.print_help()
