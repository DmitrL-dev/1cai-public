...
class AISecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not agent_config:
            logger.warning(
                f"No security config found for agent {agent_id}, allowing request but logging warning."
            )
        ...
        try:
            ...
            if user_input:
                ...
                if not security_check.allowed:
                    logger.warning(
                        f"Blocked request to {agent_id}: {security_check.reason}"
                    )
                    return JSONResponse(
                        status_code=403,
                        content={
            ...
            logger.error(
                f"Error in AI Security Middleware input validation: {e}", exc_info=True
            )
