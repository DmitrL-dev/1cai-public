"""Pilot Launchpad API for buyer rollout and procurement planning."""
...
router = APIRouter(prefix="/api/v1/pilot-launchpad", tags=["Pilot Launchpad"])


class PilotLaunchpadAssumptions(BaseModel):
...
class PilotLaunchpadRequest(BaseModel):
    ...
    assumptions: PilotLaunchpadAssumptions | None = None


def _build_report(req: PilotLaunchpadRequest) -> dict[str, Any]:
    store = store_or_none()
...
@router.post("/build")
def build(req: PilotLaunchpadRequest) -> dict[str, Any]:
    """Build buyer pilot offers, acceptance checks and procurement pack."""
...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "offers": 4,
        "acceptance_checks": 4,
        "activation_ready": pulse["purchase_status"] == "ready",
        "activation_gates": pulse["evidence"]["governance_gates"],
