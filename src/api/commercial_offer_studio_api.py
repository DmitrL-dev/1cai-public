"""Commercial Offer Studio API for buyable Rentgen packages."""
...
class CommercialOfferStudioRequest(BaseModel):
    ...
    assumptions: CommercialOfferStudioAssumptions | None = None


def _build_report(req: CommercialOfferStudioRequest) -> dict[str, Any]:
    store = store_or_none()
    ...
    return build_commercial_offer_studio(
        executive=executive,
        business_case=business_case,
        pilot_launchpad=pilot_launchpad,
...
@router.post("/build")
def build(req: CommercialOfferStudioRequest) -> dict[str, Any]:
    """Build commercial offers, price anchors, proposal sections and close path."""
...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "score": pulse["score"],
        "offers": 5,
        "deal_risks": 0 if pulse["purchase_status"] == "ready" else 1,
