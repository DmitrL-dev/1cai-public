"""Scenario Hub API for pain-led 1C Rentgen demos."""
...
router = APIRouter(prefix="/api/v1/scenario-hub", tags=["Scenario Hub"])


class ScenarioHubAssumptions(BaseModel):
...
class ScenarioHubRequest(BaseModel):
    ...
    assumptions: ScenarioHubAssumptions | None = None


def _build_report(req: ScenarioHubRequest) -> dict[str, Any]:
    store = store_or_none()
...
@router.post("/build")
def build(req: ScenarioHubRequest) -> dict[str, Any]:
    """Build the pain-led scenario gallery over live local Rentgen signals."""
...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "score": pulse["score"],
        "scenarios": 8,
        "roles": pulse["concierge"]["persona_cards"],
        "recommended": 4,
