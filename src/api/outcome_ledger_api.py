"""Outcome Ledger API for post-purchase adoption and value proof."""
...
router = APIRouter(prefix="/api/v1/outcome-ledger", tags=["Outcome Ledger"])


class OutcomeLedgerAssumptions(BaseModel):
...
class OutcomeLedgerRequest(BaseModel):
    ...
    assumptions: OutcomeLedgerAssumptions | None = None


def _build_report(req: OutcomeLedgerRequest) -> dict[str, Any]:
    store = store_or_none()
...
@router.post("/build")
def build(req: OutcomeLedgerRequest) -> dict[str, Any]:
    """Build post-purchase outcomes, adoption timeline, metrics and expansion paths."""
...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "outcome_tiles": 4,
        "success_metrics": 5,
        "governance_gates": pulse["evidence"]["governance_gates"],
        "governance_windows": 4,
