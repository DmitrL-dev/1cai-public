"""Buyer Concierge API for first-click product orientation."""
...
router = APIRouter(prefix="/api/v1/buyer-concierge", tags=["Buyer Concierge"])


class BuyerConciergeAssumptions(BaseModel):
...
class BuyerConciergeRequest(BaseModel):
    ...
    assumptions: BuyerConciergeAssumptions | None = None


def _build_report(req: BuyerConciergeRequest) -> dict[str, Any]:
    store = store_or_none()
...
@router.post("/build")
def build(req: BuyerConciergeRequest) -> dict[str, Any]:
    """Build first-click role, pain and shortest-path guidance."""
...
def health() -> dict[str, Any]:
    ...
    return {
        "status": pulse["concierge"]["status"],
        "score": pulse["concierge"]["score"],
        "persona_cards": pulse["concierge"]["persona_cards"],
        "shortest_paths": pulse["concierge"]["shortest_paths"],
        "purchase_router_status": pulse["concierge"]["purchase_router_status"],
