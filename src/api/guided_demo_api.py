"""Guided Demo API for buyer-safe product walkthroughs."""
...
class GuidedDemoAssumptions(BaseModel):
...
class GuidedDemoRequest(BaseModel):
    ...
    assumptions: GuidedDemoAssumptions | None = None


def _build_report(req: GuidedDemoRequest) -> dict[str, Any]:
    store = store_or_none()
...
@router.post("/build")
def build(req: GuidedDemoRequest) -> dict[str, Any]:
    """Build the guided buyer demo route over live local Rentgen signals."""
