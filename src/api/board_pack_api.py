"""Board Pack API for board-level Rentgen buying decisions."""
...
router = APIRouter(prefix="/api/v1/board-pack", tags=["Board Pack"])


class BoardPackAssumptions(BaseModel):
...
class BoardPackRequest(BaseModel):
    ...
    assumptions: BoardPackAssumptions | None = None


def _build_report(req: BoardPackRequest) -> dict[str, Any]:
    store = store_or_none()
...
@router.post("/build")
def build(req: BoardPackRequest) -> dict[str, Any]:
    """Build a board-level decision pack over buyer, trust, offer and proof reports."""
