...
class Provenance(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Уверенность в достоверности (0.0 - 1.0)"
    )
    timestamp: float = Field(
        default_factory=time.time, description="Время создания воспоминания"
    )
    author_agent: Optional[str] = Field(
        None, description="Имя агента, создавшего запись"
    )
...
class MemoryItem(BaseModel):
    ...
    strength: float = Field(
        1.0, description="Текущая сила воспоминания (с учетом забывания)"
    )
