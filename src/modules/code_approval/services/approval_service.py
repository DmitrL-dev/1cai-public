...
class CodeApprovalService:
    ...
    def generate_code(
        ...
        return self.agent.generate_code(
            prompt=prompt, context={"user_id": user_id, **(context or {})}
        )
    ...
    def apply_suggestion(
        self, token: str, approved_by_user: str, changes_made: str = None
    ) -> Dict[str, Any]:
        """Apply suggestion"""
    ...
    def bulk_approve_safe_suggestions(
        ...
        return self.agent.bulk_approve_safe_suggestions(
            tokens=tokens, approved_by_user=approved_by_user
        )
