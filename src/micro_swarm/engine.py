"""Autograd engine — scalar-level reverse-mode autodiff.

Ported from SENTINEL virtualconv-engine (Karpathy microgpt.py heritage).
...
class Value:
    ...
    def log(self) -> "Value":
        return Value(
            math.log(self.data + 1e-10),
            (self,),
            (1.0 / (self.data + 1e-10),),
        )

    def exp(self) -> "Value":
        e = math.exp(min(self.data, 50.0))
