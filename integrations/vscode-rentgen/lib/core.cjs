...
function profileConfig(value) {
  requireValue(value?.schema === 1 && ['0.1.0.dev4', '0.1.0.dev5', '0.1.0.dev6', '0.1.0.dev7', '0.1.0.dev8', '0.1.0.dev9'].includes(value.core_version) && uuid.test(value.project_id), 'INVALID_EDITOR_PROFILE');
...
function createClient(config, { execute, trusted = () => true }) {
  ...
  return Object.freeze({
    ...
    async bslCheck({receipt: saved, proposal, file}) {
      ...
      requireValue(['0.1.0.dev8','0.1.0.dev9'].includes(config.core_version),'BSL_REQUIRES_DEV8');
    ...
    async testProfiles() {
      requireValue(['0.1.0.dev8','0.1.0.dev9'].includes(config.core_version),'TEST_REQUIRES_DEV8');
    ...
    async testResult(id) {
      requireValue(['0.1.0.dev8','0.1.0.dev9'].includes(config.core_version),'TEST_REQUIRES_DEV8');requireValue(uuid.test(id),'INVALID_OPERATION_ID');
    ...
    async platformCheck({id, ref, contentId, proposal, platform, platformHash}) {
      requireValue(['0.1.0.dev8','0.1.0.dev9'].includes(config.core_version), 'PLATFORM_REQUIRES_DEV8');
