...
async def verify_project_manager_enhancements():
    ...
    print(
        f"Risk 1 (5x5): Score={risk1.score}, Level={risk1.level}, Strategy={risk1.strategy}"
    )
    ...
    print(
        f"Risk 2 (1x2): Score={risk2.score}, Level={risk2.level}, Strategy={risk2.strategy}"
    )
    ...
    task_python = Task(
        ...
        description="Use Python FastAPI",
        tags=["backend"],
    )
    task_sql = Task(
        id="t2",
        title="Optimize SQL Query",
        description="Fix slow join",
        tags=["database"],
    )

    agent1 = allocator.get_best_agent_for_task(task_python)
    print(
        f"Task: {task_python.title} -> Agent: {agent1.name} (Skills: {agent1.skills})"
    )
