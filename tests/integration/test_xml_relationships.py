...
from src.ai.code_analysis.graph import EdgeKind, InMemoryCodeGraphBackend, NodeKind
...
async def test_xml_relationships():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Create Catalog file
        with open(
            os.path.join(tmpdir, "Catalog_Goods.xml"), "w", encoding="utf-8"
        ) as f:
            f.write(SAMPLE_CATALOG)

        # Create Subsystem file
        with open(
            os.path.join(tmpdir, "Subsystem_Sales.xml"), "w", encoding="utf-8"
        ) as f:
