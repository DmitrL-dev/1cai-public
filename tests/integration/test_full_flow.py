...
async def test_full_flow():
    ...
    try:
        ...
        with patch("src.ai.strategies.graph.get_neo4j_client") as mock_neo4j_get, patch(
            "src.ai.strategies.graph.get_nl_to_cypher_converter"
        ) as mock_converter_get, patch(
            "src.ai.strategies.semantic.QdrantClient"
        ) as mock_qdrant, patch(
            "src.ai.strategies.semantic.EmbeddingService"
        ) as mock_embedding:
            # Setup Neo4j Mocks
            ...
            history = orchestrator.event_bus.get_event_history(
                EventType.SYSTEM_HEALTH_CHECK, limit=1
            )

            if (
                len(history) > 0
                and history[0].payload.get("message") == "Тестовое событие"
            ):
            ...
            # Mock SelfEvolvingAI
            orchestrator.evolving_ai.evolve = AsyncMock(
                return_value={
                    "status": "completed",
            ...
            if evolution_result.get("status") == "completed":
                ...
                print(
                    f"   Улучшений сгенерировано: {evolution_result.get('improvements_generated')}"
                )
            else:
