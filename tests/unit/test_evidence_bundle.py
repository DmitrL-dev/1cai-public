...
def test_evidence_bundle_builds_hashed_manifest(tmp_path, monkeypatch):
    ...
    approval_workflow.create_approval_record(
        ...
        argument_constraints={
            ...
            "modulePath": "CommonModules/Sales/Ext/Module.bsl",
        },
        expires_in_hours=12,
    )
    ...
    questionnaire_artifact = next(
        item for item in report["artifacts"] if item["id"] == "security-questionnaire"
    )
    ...
    governance_artifact = next(
        item for item in report["artifacts"] if item["id"] == "governance-proof"
    )
    ...
    killer_artifact = next(
        item for item in report["artifacts"] if item["id"] == "killer-demo"
    )
    ...
    business_artifact = next(
        item for item in report["artifacts"] if item["id"] == "business-case"
    )
    business_report = json.loads(business_artifact["json"])
    buyer_pulse_artifact = next(
        item for item in report["artifacts"] if item["id"] == "buyer-pulse"
    )
    buyer_pulse_report = json.loads(buyer_pulse_artifact["json"])
    assert (
        buyer_pulse_report["buyer_pulse"]["commercial"]["monthly_ai_rent"]
        == "200 000 RUB"
    )
    assert (
        buyer_pulse_report["buyer_pulse"]["commercial"]["three_year_ai_rent"]
        == "7 200 000 RUB"
    )
    assert (
        buyer_pulse_report["buyer_pulse"]["purchase_path"]["primary_route"]
        == "/killer-demo"
    )
    assert (
        "rentgen-buyer-room-packet.zip"
        in buyer_pulse_report["buyer_pulse"]["purchase_path"]["send_files"]
    )
    assert (
    ...
    buyer_brief_artifact = next(
        item for item in report["artifacts"] if item["id"] == "buyer-brief"
    )
    buyer_brief_report = json.loads(buyer_brief_artifact["json"])
    assert (
        buyer_brief_report["buyer_brief"]["commercial"]["monthly_ai_rent"]
        == "200 000 RUB"
    )
    assert (
        buyer_brief_report["buyer_brief"]["commercial"]["three_year_ai_rent"]
        == "7 200 000 RUB"
    )
    ...
    assert [
        item["stage"] for item in buyer_brief_report["buyer_brief"]["open_first_path"]
    ] == [
        "orient",
        "prove",
        "close",
    ...
    assert (
        buyer_brief_report["buyer_brief"]["purchase_path"]["close_artifact"]["file"]
        == "MEETING_CLOSE_RECEIPT.md"
    )
    assert (
        buyer_brief_report["buyer_brief"]["purchase_path"]["activation_artifact"][
            "file"
        ]
        == "POST_DEMO_ACTIVATION_HANDOFF.md"
    )
    assert (
        buyer_brief_report["buyer_brief"]["purchase_path"]["archive_receipt_artifact"][
            "file"
        ]
        == "archive-acceptance-receipt.md"
    )
    assert (
        buyer_brief_report["buyer_brief"]["purchase_path"][
    ...
    assert (
        ...
        in buyer_brief_report["buyer_brief"]["purchase_path"]["send_files"]
    )
    assert (
        "rentgen-buyer-room-packet.zip"
        in buyer_brief_report["buyer_brief"]["purchase_path"]["send_files"]
    ...
    assert any(
        item["role"] == "security"
        for item in buyer_brief_report["buyer_brief"]["role_cards"]
    )
    assert any(
        item["id"] == "audit-siem"
        for item in buyer_brief_report["buyer_brief"]["proof_readiness"]
    )
    assert "Buyer Brief" in buyer_brief_artifact["markdown"]
    assert "Buyer Room Plan" in buyer_brief_artifact["markdown"]
    buyer_room_plan_artifact = next(
        item for item in report["artifacts"] if item["id"] == "buyer-room-plan"
    )
    buyer_room_plan_report = json.loads(buyer_room_plan_artifact["json"])
    assert buyer_room_plan_report["decision"]["status"] == "ready"
    assert (
        buyer_room_plan_report["summary"]["evidence_contract"] == "buyer_room_plan_v1"
    )
    ...
    assert (
        buyer_room_plan_report["summary"]["proof_file"] == "rentgen-developer-report.md"
    )
    ...
    assert (
        "MEETING_CLOSE_RECEIPT.md"
        in buyer_room_plan_report["buyer_room_plan"]["send_files"]
    )
    assert (
        "Would this remove"
        in buyer_room_plan_report["buyer_room_plan"]["close_question"]
    )
    ...
    open_first_path_artifact = next(
        item for item in report["artifacts"] if item["id"] == "open-first-path"
    )
    ...
    assert open_first_path_report["summary"]["stages"] == [
        "orient",
        "prove",
        "close",
        "verify",
    ...
    board_artifact = next(
        item for item in report["artifacts"] if item["id"] == "board-pack"
    )
    ...
    assert (
        killer_report["commercial_close_packet"]["one_page_order"]["three_year_ai_rent"]
        == "7 200 000 RUB"
    )
    assert (
        killer_report["deal_readiness"]["local_asset_case"]["three_year_ai_rent"]
        == "7 200 000 RUB"
    )
    ...
    assert (
        report["commercial_assumptions"]["three_year_ai_rent_label"] == "7 200 000 RUB"
    )
    assert "7 200 000 RUB" in report["open_first_markdown"]
    assert "7 200 000 RUB" in report["procurement_handoff"]["markdown"]
    assert "buyer-brief.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "buyer-room-plan.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "buyer-room-plan.json" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "buyer-pulse.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "business-case.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "launch-room.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "killer-demo.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "test-factory.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "board-pack.md" in [item["filename"] for item in report["manifest"]["files"]]
    assert "outcome-ledger.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "buyer-concierge.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "commercial-offer-studio.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "demo-command-center.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "rentgen-developer-report.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "rentgen-architect-report.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "rentgen-director-report.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "rentgen-qa-report.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "guided-demo.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "scenario-hub.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "pilot-launchpad.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "enterprise-trust-center.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "rentgen-security-questionnaire.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "productization-readiness.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    assert "governance-proof.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
    ...
    assert any(
        item["role"] == "Director / sponsor"
        for item in killer_bridge["recipient_overlays"]
    )
    ...
    expected_receipt_status = (
        "ready"
        if handoff["ready_to_forward"] and killer_bridge["available"]
    ...
    assert any(
        item["id"] == "linked-killer-demo-archive"
        for item in archive_receipt["archives"]
    )
    assert (
        archive_receipt["control_packets"][0]["filename"]
        == "archive-verification-packet.zip"
    )
    evidence_archive = next(
        item for item in archive_receipt["archives"] if item["id"] == "evidence-archive"
    )
    linked_archive = next(
        item
        for item in archive_receipt["archives"]
        if item["id"] == "linked-killer-demo-archive"
    )
    ...
    assert (
        len(handoff["required_files"])
        == report["summary"]["procurement_required_files"]
    )
    assert any(
        item["filename"] == "commercial-offer-studio.md" and item["present"]
        for item in handoff["required_files"]
    )
    assert any(
        item["filename"] == "buyer-brief.md" and item["present"]
        for item in handoff["required_files"]
    )
    assert any(
        item["filename"] == "buyer-room-plan.md" and item["present"]
        for item in handoff["required_files"]
    )
    assert any(
        item["filename"] == "buyer-pulse.md" and item["present"]
        for item in handoff["required_files"]
    )
    assert any(
        item["filename"] == "rentgen-developer-report.md" and item["present"]
        for item in handoff["required_files"]
    )
    assert any(
        item["filename"] == "rentgen-architect-report.md" and item["present"]
        for item in handoff["required_files"]
    )
    assert any(
        item["filename"] == "rentgen-director-report.md" and item["present"]
        for item in handoff["required_files"]
    )
    assert any(
        item["filename"] == "rentgen-security-questionnaire.md" and item["present"]
        for item in handoff["required_files"]
    )
    assert any(
        item["role"] == "Security / procurement" for item in handoff["recipients"]
    )
    director_recipient = next(
        item for item in handoff["recipients"] if item["role"] == "Director / sponsor"
    )
    ...
    security_recipient = next(
        item
        for item in handoff["recipients"]
        if item["role"] == "Security / procurement"
    )
    ...
    director_packet = next(
        item
        for item in handoff["recipient_packets"]
        if item["role"] == "Director / sponsor"
    )
    assert director_packet["filename"] == "ROLE_DIRECTOR_SPONSOR.md"
    assert (
        director_packet["forwarding_filename"]
        == "ROLE_DIRECTOR_SPONSOR-forwarding-note.txt"
    )
    ...
    assert director_packet["forwarding_subject"].endswith(
        "evidence packet for Director / sponsor"
    )
    ...
    assert (
        "ROLE_SECURITY_PROCUREMENT-forwarding-note.txt" in handoff["archive_contents"]
    )
    ...
    assert (
        archive["archive_manifest_files"] == archive["archive_manifest"]["file_count"]
    )
    assert archive["files"] >= report["summary"]["files"] + 5
    verified = verify_evidence_bundle_archive_payload(
        archive["bytes"], filename=archive["filename"]
    )
    ...
    with zipfile.ZipFile(BytesIO(archive["bytes"])) as zip_file:
        ...
        archive_manifest = json.loads(
            zip_file.read("archive-manifest.json").decode("utf-8")
        )
        assert (
            archive_manifest["schema_version"] == "rentgen.evidence_archive_manifest.v1"
        )
        assert archive_manifest["source_manifest_file"] == "manifest.json"
        assert (
            archive_manifest["source_manifest_bundle_sha256"]
            == report["manifest"]["bundle_sha256"]
        )
        archive_manifest_files = {
            item["filename"]: item for item in archive_manifest["files"]
        }
        ...
        assert (
            archive_manifest_files["ROLE_DIRECTOR_SPONSOR.md"]["sha256"]
            == hashlib.sha256(zip_file.read("ROLE_DIRECTOR_SPONSOR.md")).hexdigest()
        )
        assert (
            archive_manifest_files["ROLE_DIRECTOR_SPONSOR-forwarding-note.txt"][
                "sha256"
            ]
            == hashlib.sha256(
        ...
        archive_handoff = json.loads(
            zip_file.read("procurement-handoff.json").decode("utf-8")
        )
        assert archive_handoff["bundle_id"] == report["bundle_id"]
        assert (
            archive_handoff["commercial_assumptions"]["three_year_ai_rent_label"]
            == "7 200 000 RUB"
        )
        assert (
            archive_handoff["killer_demo_handoff"]["archive_endpoint"]
        ...
        director_forwarding_note = zip_file.read(
            "ROLE_DIRECTOR_SPONSOR-forwarding-note.txt"
        ).decode("utf-8")
        assert director_forwarding_note.startswith(
            "Subject: ACME: 1C Rentgen evidence packet"
        )
        ...
        archive_receipt_json = json.loads(
            zip_file.read("archive-acceptance-receipt.json").decode("utf-8")
        )
        assert archive_receipt_json["archives"][0]["hash_header"] == "X-Archive-Sha256"
        assert (
            archive_receipt_json["archives"][0]["archive_manifest_file"]
            == "archive-manifest.json"
        )
        assert "VERIFY_ARCHIVE.md" in archive_receipt_json["archives"][0]["contains"]
        assert (
            "ROLE_DIRECTOR_SPONSOR-forwarding-note.txt"
            in archive_receipt_json["archives"][0]["contains"]
        )
        assert (
            archive_receipt_json["archives"][1]["hash_header"]
            == "X-Killer-Demo-Archive-Sha256"
        )
        archive_receipt_md = zip_file.read("archive-acceptance-receipt.md").decode(
            "utf-8"
        )
    ...
    tampered_verified = verify_evidence_bundle_archive_payload(
        tampered.getvalue(), filename=archive["filename"]
    )
    assert tampered_verified["status"] == "fail"
    assert (
        tampered_verified["verification_receipt"]["decision"]["status"]
        == "reject_archive"
    )
    assert any(
        item["code"] == "archive-entry-hash-mismatch"
        for item in tampered_verified["findings"]
    )
...
def test_evidence_bundle_can_include_extension_safety(tmp_path):
    ...
    artifact = next(
        item for item in report["artifacts"] if item["id"] == "extension-safety"
    )
    assert artifact["status"] == "risk"
    assert artifact["summary"]["extensions"] == 1
    assert "extension-safety.md" in [
        item["filename"] for item in report["manifest"]["files"]
    ]
...
def test_evidence_bundle_archive_api_exposes_archive_manifest_headers(
    ...
    response = client.post(
        "/api/v1/evidence-bundle/archive", json={"client_name": "ACME"}
    )
    ...
    assert (
        int(response.headers["x-archive-manifest-files"]) >= report["summary"]["files"]
    )
    with zipfile.ZipFile(BytesIO(response.content)) as zip_file:
        manifest_hash = hashlib.sha256(
            zip_file.read("archive-manifest.json")
        ).hexdigest()
    assert response.headers["x-archive-manifest-sha256"] == manifest_hash

    verify_response = client.post(
        "/api/v1/evidence-bundle/archive/verify", json={"client_name": "ACME"}
    )
    ...
    assert verified["status"] == "pass"
    assert (
        verified["archive_sha256"] == verified["expected_headers"]["X-Archive-Sha256"]
    )
    ...
    assert (
        verified["verification_receipt"]["expected_headers"]["X-Archive-Manifest"]
        == "archive-manifest.json"
    )
    assert (
        verified["verification_receipt"]["receipt_files"]["markdown"]
        == "archive-verification-receipt.md"
    )
    assert "X-Archive-Manifest-Sha256" in verified["verification_receipt_markdown"]

    dual_response = client.post(
        "/api/v1/evidence-bundle/archive/verify-dual", json={"client_name": "ACME"}
    )
    ...
    assert (
        dual["pair"]["evidence_archive_sha256"]
        == dual["killer_demo"]["expected_headers"]["X-Evidence-Archive-Sha256"]
    )
    assert (
        dual["evidence"]["verification_receipt"]["decision"]["status"]
        == "ready_to_record"
    )
    assert (
        dual["killer_demo"]["verification_receipt"]["embedded_evidence"]["status"]
        == "pass"
    )
    ...
    assert (
        packet_response.headers["x-verification-packet-sha256"]
        == hashlib.sha256(packet_response.content).hexdigest()
    )
    with zipfile.ZipFile(BytesIO(packet_response.content)) as packet_zip:
        ...
        packet_json = json.loads(
            packet_zip.read("dual-archive-verification-packet.json").decode("utf-8")
        )
    ...
    assert any(
        item["filename"] == "dual-archive-verification-packet.md"
        for item in hash_table["packet_files"]
    )
