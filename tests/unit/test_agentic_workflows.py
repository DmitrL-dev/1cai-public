...
def test_agentic_plan_review_and_action_gate(tmp_path, monkeypatch):
    ...
    artifact_graph.create_artifact(
        {"id": "REQ-AGENT", "type": "requirement", "title": "Agent source"},
        path=artifacts,
    )
    ...
    plan = agentic_workflows.create_agentic_plan(
        {
            ...
            "steps": [
                {"title": "Write BSL", "action_type": "bsl_write", "risk": "high"}
            ],
        },
        path=store,
        artifact_path=artifacts,
    )
    reviewed = agentic_workflows.review_agentic_plan(
        plan["id"], path=store, artifact_path=artifacts
    )
    blocked = agentic_workflows.action_gate(
        action_type="bsl_write", plan_id=plan["id"], risk="high", path=store
    )
    allowed = agentic_workflows.action_gate(
        action_type="analysis", risk="low", path=store
    )

    assert reviewed["review"]["status"] == "fail"
    assert any(
        item["code"] == "act-without-approved-plan"
        for item in reviewed["review"]["findings"]
    )
...
def test_agentic_api_exposes_plan_review_and_gate(tmp_path, monkeypatch):
    monkeypatch.setattr(
        agentic_workflows, "STORE_PATH", tmp_path / "agentic_workflows.json"
    )
    ...
    gate = client.post(
        "/api/v1/agentic/action-gate",
        json={"action_type": "metadata_write", "plan_id": "PLAN-API", "risk": "high"},
    )
...
async def test_mcp_agentic_tools_are_registered_and_work(tmp_path, monkeypatch):
    ...
    plan = await handle_agentic_plan_create(
        {"id": "PLAN-MCP", "title": "MCP plan", "mode": "plan"}
    )
    review = await handle_agentic_plan_review({"plan_id": plan["id"]})
    gate = await handle_agentic_action_gate(
        {"action_type": "external_command", "plan_id": plan["id"], "risk": "high"}
    )
