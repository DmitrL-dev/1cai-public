...
def _scenario_hub():
    return {
        ...
        "scenarios": [
            {"id": "left-join-null", "title": "LEFT JOIN field without ЕстьNULL", "pain": "NULL risk", "buyer_line": "Fix it.", "route": "/quality"},
            {"id": "local-enterprise-proof", "title": "Local enterprise proof", "pain": "AI rent", "buyer_line": "Buy local.", "route": "/business-case"},
        ],
        "recommended_path": [
            {"scenario_id": "left-join-null", "title": "LEFT JOIN field without ЕстьNULL", "route": "/quality", "why": "real defect"},
            {"scenario_id": "local-enterprise-proof", "title": "Local enterprise proof", "route": "/business-case", "why": "local asset"},
        ],
    }
...
def _buyer_brief(status="ready", score=88):
    return {
        ...
        "meeting_flow": [
            ...
            {"step": 3, "label": "Activate", "route": "/pilot-launchpad", "line": "Start paid activation."},
        ...
        "open_first_path": [
            ...
            {"step": 3, "stage": "close", "label": "Close", "title": "Activation", "route": "/pilot-launchpad", "line": "Start paid activation.", "file": "POST_DEMO_ACTIVATION_HANDOFF.md", "status": status, "source": "test"},
...
def test_pilot_launchpad_builds_offers_acceptance_and_procurement_pack():
    ...
    assert report["activation_contract"]["selected_offer_id"] == "enterprise-local-license"
    assert report["activation_contract"]["ready_to_activate"] is True
    assert any(item["filename"] == "rentgen-buyer-room-packet.zip" for item in report["activation_contract"]["handoff_files"])
    assert any(item["filename"] == "MEETING_CLOSE_RECEIPT.md" for item in report["activation_contract"]["handoff_files"])
    assert any(item["filename"] == "POST_DEMO_ACTIVATION_HANDOFF.md" for item in report["activation_contract"]["handoff_files"])
    assert any(item["filename"] == "archive-acceptance-receipt.md" for item in report["activation_contract"]["handoff_files"])
    assert any(item["filename"] == "archive-verification-packet.zip" for item in report["activation_contract"]["handoff_files"])
    ...
    assert [item["stage"] for item in pilot_room["open_first_path"]] == ["orient", "prove", "close", "verify"]
    ...
    assert "MEETING_CLOSE_RECEIPT.md" in [item["filename"] for item in report["exports"]]
    assert "archive-acceptance-receipt.json" in [item["filename"] for item in report["exports"]]
    assert "archive-verification-packet.zip" in [item["filename"] for item in report["exports"]]
    ...
    assert {item["route"] for item in report["activation_contract"]["buyer_commitments"]} >= {
        "/business-case",
        "/approvals",
        "/evidence-bundle",
    }
    assert any(item["role"] == "Developer" for item in report["acceptance_matrix"])
    assert any(item["artifact"] == "Buyer Room Packet ZIP" for item in report["procurement_pack"])
    assert any(item["owner"] == "Security" for item in report["procurement_pack"])
    assert any(item["artifact"] == "Archive Acceptance Receipt" for item in report["procurement_pack"])
    assert any(item["artifact"] == "Verification Packet ZIP" for item in report["procurement_pack"])
    ...
    assert any(item["id"] == "day0-owner-scope" and item["status"] == "ready" for item in register["items"])
    assert any(item["id"] == "day0-close-packet" and "rentgen-buyer-room-packet.zip" in item["acceptance"] for item in register["items"])
    assert any(item["id"] == "day0-close-packet" and item["evidence_file"] == "archive-acceptance-receipt.md" for item in register["items"])
    assert any(item["id"] == "day0-verification-packet" and item["evidence_file"] == "archive-verification-packet.zip" for item in register["items"])
    assert any(item["id"] == "day30-conversion" and item["evidence_route"] == "/outcome-ledger" for item in register["items"])
...
def test_pilot_launchpad_acceptance_register_blocks_productization_failure():
    ...
    assert [item["stage"] for item in report["pilot_room_bridge"]["open_first_path"]] == [
        "orient",
        "prove",
        "close",
        "verify",
    ...
    assert any(item["status"] == "blocked" and item["role"] == "Architect" for item in register["items"])
