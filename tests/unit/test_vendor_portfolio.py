...
def test_vendor_portfolio_builds_pre_sale_work_packages():
    ...
    assert report["portfolio"]["coverage_caveats"] == report["coverage_ledger"]["summary"]["caveats"]
    ...
    assert report["vendor_room_bridge"]["files"][:3] == ["buyer-brief.md", "buyer-pulse.md", "open-first-path.md"]
    assert [item["stage"] for item in report["vendor_room_bridge"]["open_first_path"]] == ["orient", "prove", "close", "verify"]
