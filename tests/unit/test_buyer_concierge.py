...
def test_buyer_concierge_builds_first_click_guidance():
    report = build_buyer_concierge(
        ...
        vendor_portfolio={
            "decision": {"status": "ready", "score": 80},
            "work_packages": [{"id": "audit"}],
        },
    ...
    assert {item["id"] for item in report["persona_cards"]} >= {
        "developer",
        "architect",
        "director",
        "security",
    ...
    director = next(
        item for item in report["persona_cards"] if item["id"] == "director"
    )
    ...
    assert (
        report["purchase_router"]["recommended_purchase"] == "Enterprise local license"
    )
    ...
    assert report["concierge_room_bridge"]["files"][:2] == [
        "buyer-brief.md",
        "buyer-pulse.md",
    ]
    ...
    assert any(
        item["scenario_id"] == "left-join-null" for item in report["pain_picker"]
    )
