...
def test_artifact_update_preserves_attributes_and_versions(tmp_path):
    ...
    item = graph.create_artifact(
        {"type": "requirement", "title": "Posting validation", "attributes": {"source": "ba"}},
        path=store,
    )
...
def test_artifact_create_upserts_explicit_id_without_losing_history(tmp_path):
    ...
    first = graph.create_artifact(
        {"id": "REQ-1", "type": "requirement", "title": "Posting validation", "owner": "ba"},
        path=store,
    )
    second = graph.create_artifact(
        {"id": "REQ-1", "type": "requirement", "title": "Posting validation", "owner": "architect"},
        path=store,
    )
...
def test_artifact_graph_rejects_unknown_types(tmp_path):
    with pytest.raises(ValueError):
        graph.create_artifact({"type": "unknown", "title": "Bad"}, path=tmp_path / "artifact_graph.json")
...
async def test_mcp_artifact_tools_are_registered_and_work(tmp_path, monkeypatch):
    ...
    assert {"artifact_create", "artifact_link", "artifact_trace", "artifact_matrix"} <= names

    requirement = await handle_artifact_create({"type": "requirement", "title": "Credit limit"})
    test_case = await handle_artifact_create({"type": "test_case", "title": "YAxUnit credit limit"})
