    ...
    return {
        ...
        "subscription_escape_plan": {
            ...
            "stakeholder_lines": [
                {
                    "role": "Finance",
                    "line": "Compare rent and local license.",
                    "route": "/business-case",
                }
            ],
            "evidence_files": [
...
def test_commercial_offer_studio_builds_buyable_offer_stack():
    ...
    assert (
        report["procurement_dossier"]["recommended_purchase"]["id"]
        == "enterprise-local-license"
    )
    assert (
        report["procurement_dossier"]["subscription_escape"]["annual_ai_rent"]
        == "1 440 000 RUB"
    )
    assert (
        report["procurement_dossier"]["subscription_escape"]["three_year_ai_rent"]
        == "4 320 000 RUB"
    )
    assert (
        report["procurement_dossier"]["subscription_escape"]["local_value_anchor"]
        == "4 800 000 RUB"
    )
    assert (
        report["procurement_dossier"]["subscription_escape"]["local_license_anchor"]
        == "1 800 000 RUB"
    )
    assert (
        report["procurement_dossier"]["subscription_escape"]["break_even_months"] == 5
    )
    ...
    assert any(
        item["role"] == "finance"
        for item in report["procurement_dossier"]["approval_matrix"]
    )
    assert {
        item["model"] for item in report["procurement_dossier"]["license_model"]
    } >= {
        "per installation",
        "per configuration",
        "portfolio",
    ...
    assert offer_room["files"][:3] == [
        "buyer-brief.md",
        "buyer-pulse.md",
        "open-first-path.md",
    ]
    assert [item["stage"] for item in offer_room["open_first_path"]] == [
        "orient",
        "prove",
        "close",
        "verify",
    ...
    assert any(
        item["route"] == "/commercial-offer-studio" for item in report["exports"]
    )
...
def test_commercial_offer_studio_keeps_caveats_visible_when_trust_is_risky():
    ...
    assert report["offer_room_bridge"]["files"][:3] == [
        "buyer-brief.md",
        "buyer-pulse.md",
        "open-first-path.md",
    ]
