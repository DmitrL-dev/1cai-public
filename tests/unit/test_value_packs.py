...
def test_value_packs_turn_features_into_buyer_outcomes():
    ...
    assert report["value_room_bridge"]["files"][:3] == ["buyer-brief.md", "buyer-pulse.md", "open-first-path.md"]
    assert [item["stage"] for item in report["value_room_bridge"]["open_first_path"]] == ["orient", "prove", "close", "verify"]
