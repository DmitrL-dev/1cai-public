from src.services.rentgen.business_case import (
    build_business_case,
...
def test_business_case_builds_money_map_and_buyer_committee():
    report = build_business_case(
        ...
        value_packs={
            "packs": [{"title": "Developer Pack"}, {"title": "Platform Pack"}]
        },
        client_name="ACME",
    ...
    assert report["business_room_bridge"]["files"][:3] == [
        "buyer-brief.md",
        "buyer-pulse.md",
        "open-first-path.md",
    ]
    assert [
        item["stage"] for item in report["business_room_bridge"]["open_first_path"]
    ...
    assert any(
        item["role"] == "Finance"
        for item in report["subscription_escape_plan"]["stakeholder_lines"]
    )
    assert any(
        item["filename"] == "OPEN_FIRST.md"
        for item in report["subscription_escape_plan"]["evidence_files"]
    )
    assert any(
        item["id"] == "subscription-displacement" for item in report["business_levers"]
    )
    assert any(
        item["role"] == "Director" and item["route"] == "/business-case"
